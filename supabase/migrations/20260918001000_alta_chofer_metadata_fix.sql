-- =====================================================================
-- 0016 · Fix metadata y excepciones
--
-- 1. Se remueve el uso de "metadata" en choferes (no existía en el schema).
-- 2. Al forzar alta (lista_negra), se crea un registro aprobado en `excepciones`
--    para que aparezca en el listado de auditoría, tal como pidió el usuario.
-- 3. Se elimina la restricción de base de datos que bloqueaba la auto-aprobación,
--    ya que RLS ya había sido relajado pero el check fallaba.
-- =====================================================================

alter table excepciones drop constraint if exists ck_excepcion_aprobador;
...
begin
  ...
  -- Si forzamos lista negra, creamos la excepción aprobada directamente
  if v_forzar then
  ...
  -- Chofer (original) - usamos observaciones para alertas json (opcional, pero sin columna metadata)
